...
/*
 * Edited by Paulo Sherring.
 * Changed a few mapping values that were wrong.
 * Added DigitalWrite and DigitalRead functions.
 */
...
//                     1  2  3  4  5  6  7  8
const char p8_bank[]={-1,-1, 1, 1, 1, 1, 2, 2,	//8
                       2, 2, 1, 1, 0, 0, 1, 1, 	//16
                       0, 2, 0, 1, 1, 1, 1, 1,	//24
                       1, 1, 2, 2, 2, 2, 0, 0,  //32
                       0, 2, 0, 2, 2, 2, 2, 2,	//40
...
/*	     1      2          3           4           5           6           7           8 	*/
	     0,		0,		1<<6,		1<<7,		1<<2,		1<<3,		1<<2,		1<<3, 	//8
      1<<5,	 1<<4,     1<<13,      1<<12,      1<<23,      1<<26,      1<<15,      1<<14,	//16
     1<<27,  1<<1,     1<<22,      1<<31,      1<<30,       1<<5,       1<<4,       1<<1,	//24
      1<<0, 1<<29,     1<<22,      1<<24,      1<<23,      1<<25,      1<<10,      1<<11,	//32
      1<<9, 1<<17,      1<<8,      1<<16,      1<<14,      1<<15,      1<<12,      1<<13,	//40
     1<<10, 1<<11,      1<<8,       1<<9,       1<<6,       1<<7};							//46


//                       1  2  3  4  5  6  7  8
const char p9_bank[]={  -1,-1,-1,-1,-1,-1,-1,-1, 	//8
	                    -1,-1, 0, 1, 0, 1, 1, 1,   	// 16
	                     0, 0, 0, 0, 0, 0, 1, 0,   	// 24
	                     3, 0, 3, 3, 3, 3, 3,-1,   	// 32
	                    -1,-1,-1,-1,-1,-1,-1,-1,   	// 40
...
/*	   1      2      3      4      5      6      7      8 	*/
	   0,     0,     0,     0,     0,     0,     0,     0, 	// 8
       0,     0, 1<<30, 1<<28, 1<<31, 1<<18, 1<<16, 1<<19, 	// 16
    1<<5,  1<<4, 1<<13, 1<<12,  1<<3,  1<<2, 1<<17, 1<<15,	// 24
   1<<21, 1<<14, 1<<19, 1<<17, 1<<15, 1<<16, 1<<14,     0,	// 32
       0,     0,     0,     0,     0,     0,     0,     0,	// 40
   1<<20,  1<<7,     0,     0,     0,     0};				// 46
...
inline void DigitalWrite(char port, char pin,char val)
{
	if(val)
		pin_high(port,pin);
	else
...
inline char DigitalRead(char port, char pin)
{
